...
export default async function Page({ params }) {
  ...
  return (
    <main className="flex flex-col ">
      <header>
        <img src={MediaUrl + Blog.image} className="w-full h-[550px] object-cover" alt="" />
      </header>
          ...
          Blog.sections.map((section, index) => {
            return (
              <article key={index} className="w-full flex flex-col gap-4 ">
                {section.image && section.image.trim() !== "" && <img src={MediaUrl + section.image} className="w-full rounded-xl h-72 md:h-[500px] object-cover" alt="" />}
